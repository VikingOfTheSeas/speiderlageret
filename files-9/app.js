...
// Lag QR canvas asynkront og returner Promise med canvas element
function lagQrCanvas(url, size) {
  return new Promise(resolve => {
    const wrap = document.createElement("div");
    wrap.style.cssText = "position:absolute;left:-9999px;top:-9999px";
    document.body.appendChild(wrap);
    new QRCode(wrap, { text: url, width: size, height: size, colorDark: "#000", colorLight: "#fff", correctLevel: QRCode.CorrectLevel.H });
    // QRCode.js renders synchronously to canvas, but we wait one tick
    setTimeout(() => {
      const canvas = wrap.querySelector("canvas");
      const img    = wrap.querySelector("img");
      const result = canvas || img;
      document.body.removeChild(wrap);
      resolve(result ? result.outerHTML : "");
    }, 80);
  });
}
...
async function skrivUtEtikett() {
  ...
  const qrHTML = await lagQrCanvas(url, 106);
