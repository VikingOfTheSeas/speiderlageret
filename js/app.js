...
async function batchUtlan() {
  ...
  // Open utlan modal for first item, apply to all selected
  var til = prompt("Utlån til (navn/gruppe):");
  if (!til || !til.trim()) return;
  til = til.trim();
  var dato = new Date().toISOString().split("T")[0];
  for (var id of ids) {
    var g = gjenstander.find(function(x) { return x.id === id; });
    if (g) await db.from("gjenstander").upsert(Object.assign({}, g, {
      status: "Utlånt", utlant_til: til, utlansdato: dato, innleveringsdato: ""
    }), {onConflict:"id"});
  }
  clearValg(); await lastInn(); visBanner(ids.length + " satt til Utlånt", "success");
}
