...
function visTabell(liste) {
  ...
  tbody.innerHTML = liste.map(g => {
    ...
    return `<tr class="item-row row-${sk(g.status)}${sel ? " row-selected" : ""}" data-id="${g.id}">
      ...
      <td class="col-extra small-text">${g.utlant_til || '<span class="dash">—</span>'}</td>
      <td class="col-extra muted-text">${g.kategori || "—"}</td>
      <td class="col-extra mono muted-text small-text">${g.serienummer || "—"}</td>
      <td class="col-extra mono small-text">${g.hylleplassering || '<span class="dash">—</span>'}</td>
    </tr>`;
  }).join("");
}
...
async function skrivUtEtikett() {
  ...
  const qrHTML = await lagQrCanvas(url, 190);
...
async function skrivUtAlle() {
  ...
  // Split into pages of 8 (4 cols × 2 rows on A4 landscape)
  const PER_SIDE = 8;
  const sider = [];
  ...
  document.getElementById("printArea").innerHTML = sider.map((side, idx) =>
    `<div class="print-side${idx < sider.length - 1 ? " print-side-break" : ""}">
      ${side.join("")}
    </div>`
  ).join("");
