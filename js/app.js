...
function visTabell(liste) {
  ...
  tbody.innerHTML = liste.map(g => {
    ...
    return `<tr class="item-row row-${sk(g.status)}${sel ? " row-selected" : ""}" data-id="${g.id}">
      <td><input type="checkbox" class="row-checkbox" value="${g.id}" onchange="oppdaterValg()" ${sel ? "checked" : ""}></td>
...
async function startKamera() {
  try {
    ...
    await video.play();
    document.getElementById("scannerStatus").textContent = "Scan en QR-kode…";
    scannerInterval = setInterval(sjekkFrame, 300);
  } catch (e) {
    document.getElementById("scannerStatus").textContent = "❌ Kamera ikke tilgjengelig: " + e.message;
  }
}
...
function sjekkFrame() {
  ...
  const url = code.data;
  // Parse item ID from URL: item.html?id=XXX
  const match = url.match(/[?&]id=([^&]+)/);
  if (!match) {
    leggTilScanLog("Ukjent QR: " + url.slice(0, 40), true);
    return;
  }

  const id = decodeURIComponent(match[1]);
  if (skannede.has(id)) return; // Already scanned
  skannede.add(id);

  const g = gjenstander.find(x => x.id === id);
  if (!g) {
    leggTilScanLog("Ukjent ID: " + id, true);
    return;
  }

  valgte.add(id);
  leggTilScanLog("✓ " + id + " – " + g.navn, false);
  document.getElementById("scanCount").textContent = skannede.size;

  // Brief flash
  document.getElementById("scannerStatus").textContent = "✓ " + g.navn;
  setTimeout(() => {
    if (document.getElementById("scannerModal").classList.contains("open"))
      document.getElementById("scannerStatus").textContent = "Scan neste…";
  }, 800);
}

function leggTilScanLog(tekst, feil) {
...
function lukkScanner(visValgt) {
  ...
  if (visValgt && valgte.size > 0) {
    filtrerOgVis(); // Re-render with selections highlighted
    oppdaterValg();
...
function lagEtikettHTML(id, url) {
  const g = gjenstander.find(x => x.id === id);
  if (!g) return "";
  const qrDiv = document.createElement("div");
  new QRCode(qrDiv, { text: url, width: 106, height: 106, colorDark: "#000", colorLight: "#fff", correctLevel: QRCode.CorrectLevel.H });
  return `<div class="print-label">
    <div class="print-label-qr">${qrDiv.innerHTML}</div>
...
function leggIKo() {
  ...
  if (!g) return;
  if (printKo.find(x => x.id === _qrAktivId)) {
    visBanner("Allerede i kø", "error"); return;
  }
  printKo.push({ id: _qrAktivId, url: APP_BASE_URL + "item.html?id=" + enc(_qrAktivId) });
  oppdaterKoBar();
  visBanner("➕ Lagt til i kø (" + printKo.length + " stk)", "success");
}

function oppdaterKoBar() {
  const bar = document.getElementById("printQueueBar");
  document.getElementById("printQueueCount").textContent = printKo.length;
  bar.style.display = printKo.length > 0 ? "flex" : "none";
}

function skrivUtAlle() {
  if (!printKo.length) return;
  const area = document.getElementById("printArea");
  area.innerHTML = printKo.map(item => lagEtikettHTML(item.id, item.url)).join("");
  window.print();
}
