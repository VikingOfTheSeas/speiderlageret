...
return `<tr class="${f ? "row-forfalt" : ""}"> <td class="mono" style="padding-left:16px"><a class="item-link" href="item.html?id=${enc(g.id)}">${g.id}</a></td> <td>${g.navn}</td><td>${g.utlant_til || "—"}</td> <td class="mono small-text center">${formatDato(g.utlansdato)}</td> <td class="mono small-text center ${f ? "orange-text fw" : ""}">${formatDato(g.innleveringsdato)}${f ? " ⚠️" : ""}</td> <td class="center">${f ? '<span class="badge-forfalt">FORFALT</span>' : "OK"}</td> <td class="center"><button class="btn-action btn-edit" onclick="apneRediger('${g.id}')">✏️</button></td> </tr>`;
}).join(””);
}
...
return `<tr class="item-row"> <td class="tc mono"><a class="item-link" href="boks.html?id=${enc(b.id)}">${b.id}</a></td> <td class="tc"><a class="item-name-link" href="boks.html?id=${enc(b.id)}">${b.navn}</a></td> <td class="tc muted-text">${b.hylleplassering || "—"}</td> <td class="tc">${gjTekst}</td> <td class="tc"> <div class="actions-cell"> <button class="btn-action btn-edit" onclick="window.location.href='boks.html?id=${enc(b.id)}'">📋 Åpne</button> <button class="btn-action btn-qr"   onclick="apneBoksQr('${b.id}','${b.navn.replace(/'/g,"\'")}')">⬛ QR</button> <button class="btn-action btn-delete" onclick="bekreftSlettBoks('${b.id}','${b.navn.replace(/'/g,"\'")}')">🗑️</button> </div> </td> </tr>`;
}).join(””);
}

function apneNyBoks() {
document.getElementById(“nyBoksHylle”).value = “”;
document.getElementById(“nyBoksId”).value = “”;
document.getElementById(“nyBoksNavn”).value = “”;
document.getElementById(“nyBoksBeskrivelse”).value = “”;
document.getElementById(“nyBoksBildeUrl”).value = “”;
const prev = document.getElementById(“nyBoksBildePrev”);
if (prev) { prev.style.display = “none”; }
const st = document.getElementById(“nyBoksBildeStatus”);
if (st) st.style.display = “none”;
const inp = document.getElementById(“nyBoksBildeInput”);
if (inp) inp.value = “”;
window._nyBoksBildeData = null;
document.getElementById(“nyBoksModal”).classList.add(“open”);
setTimeout(() => document.getElementById(“nyBoksHylle”).focus(), 100);
}
function lukkNyBoks() { document.getElementById(“nyBoksModal”).classList.remove(“open”); }

function genererBoksId() {
const hylle = (document.getElementById(“nyBoksHylle”).value || “”).trim().toUpperCase().replace(/\s+/g, “”);
const idEl = document.getElementById(“nyBoksId”);
if (!hylle) { idEl.value = “”; return; }
const m = hylle.match(/^([A-Z]+)(\d+)$/);
let base = m ? “B-” + m[1] + m[2].padStart(2, “0”) : “B-” + hylle.replace(/[^A-Z0-9]/g, “”).slice(0, 6);
let id = base, n = 2;
while (bokser.find(b => b.id === id)) { id = base + “-” + n; n++; }
idEl.value = id;
}

async function forhandsvisBoksBilde(input) {
const fil = input.files[0]; if (!fil) return;
const st = document.getElementById(“nyBoksBildeStatus”);
...
if (st) { st.className = “bilde-upload-status success”; st.textContent = “✓ Klar!”; setTimeout(() => st.style.display = “none”, 2000); }
} catch(e) { if (st) { st.className = “bilde-upload-status feil”; st.textContent = “❌ “ + e.message; } }
}

async function lagreNyBoks() {
const id    = document.getElementById(“nyBoksId”).value.trim();
...
const bildeUrl = document.getElementById(“nyBoksBildeUrl”).value.trim();
const bilde = window._nyBoksBildeData || (bildeUrl ? konverterBildeUrl(bildeUrl) : null);
if (!hylle) { visBanner(“Hylleplassering er påkrevd”, “error”); return; }
if (!id)    { visBanner(“ID mangler — fyll inn hylleplassering”, “error”); return; }
if (!navn)  { visBanner(“Navn er påkrevd”, “error”); return; }
const btn = document.querySelector(”#nyBoksModal .btn-save”);
btn.disabled = true; btn.textContent = “Lagrer…”;
const { error } = await db.from(“bokser”).insert({ id, navn, hylleplassering: hylle, beskrivelse: beskr || null, bilde_url: bilde || null });
if (error) { visBanner(“Feil: “ + error.message, “error”); }
...
let maks = 0;
gjenstander.forEach(g => {
const m = g.id.match(new RegExp(”^” + pref.replace(/[.*+?^${}()|[]\]/g, “\$&”) + “-(\d+)$”, “i”));
...
if (!hylleEl) return;
if (boks?.hylleplassering) hylleEl.value = boks.hylleplassering;
...
const startEl = document.getElementById(“itemStartNr”);
