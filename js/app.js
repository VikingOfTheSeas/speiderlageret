...
async function batchUtlan() {
  ...
  // Reuse edit modal for first item, but apply to all
  var g = gjenstander.find(function(x) { return x.id === ids[0]; });
  if (!g) return;
  editingId = ids[0];
  window._batchUtlanIds = ids;
  document.getElementById("editModalTitle").textContent = "Utlån for " + ids.length + " gjenstander";
  document.getElementById("editNavn").value             = ids.length === 1 ? g.navn : ids.length + " gjenstander";
  document.getElementById("editKategori").value         = g.kategori || "Annet";
  document.getElementById("editSerienummer").value      = "";
  document.getElementById("editHylle").value            = g.hylleplassering || "";
  document.getElementById("editBildeUrl").value         = "";
  document.getElementById("editStatus").value           = "Utlånt";
  document.getElementById("editUtlantTil").value        = "";
  document.getElementById("editUtlansdato").value       = new Date().toISOString().split("T")[0];
  document.getElementById("editInnlevering").value      = "";
  document.getElementById("editNotater").value          = "";
  document.getElementById("editBoksId").value           = g.boks_id || "";
  document.getElementById("editBildePrev").style.display = "none";
  document.getElementById("editBildeIngen").style.display = "flex";
  document.getElementById("editBildeIngen").textContent = ids.length + " gjenstander valgt";
  toggleUtlan("Utlånt");
