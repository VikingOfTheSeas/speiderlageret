...
function renderBoks() {
  ...
  // ── Innhold-tabell: én rad per navnegruppe ─────────────────────────────
  ...
  if (!grupper.size) {
    ...
    tbody.innerHTML = [...grupper.entries()].map(([navn, gr]) => {
      const sn = navn.replace(/'/g, "\\'");
      return `<tr>
        <td style="padding-left:14px"><a class="item-name-link" href="item.html?id=${enc(gr.alle[0].id)}">${navn}</a></td>
        ...
        <td class="tc" style="white-space:nowrap">
          <button class="btn-action btn-edit" style="padding:8px 12px;font-size:16px;min-width:40px" onclick="apneUtlanForNavn('${sn}')">📤</button>
          <button class="btn-action btn-qr"  style="padding:8px 12px;font-size:16px;min-width:40px" onclick="apneLeverForNavn('${sn}')">📥</button>
        </td>
      </tr>`;
    }).join("");
  }
  ...
  if (!utlånte.length) {
    uTbody.innerHTML = '<tr><td colspan="5" class="empty-row">Ingen aktive utlån ✅</td></tr>';
  } else {
    const utGrupper = new Map();
    for (const g of utlånte) {
      const key = (g.navn || "") + "|" + (g.utlant_til || "");
      if (!utGrupper.has(key)) {
        utGrupper.set(key, {
          navn: g.navn,
          utlant_til: g.utlant_til || "—",
          innleveringsdato: g.innleveringsdato || "—",
          ids: [], antall: 0
        });
      }
      const gr = utGrupper.get(key);
      gr.ids.push(g.id);
      gr.antall++;
    }

    uTbody.innerHTML = [...utGrupper.values()].map(gr => {
      ...
      return `<tr class="${forfalt ? "row-forfalt" : ""}">
        <td style="padding-left:16px">
          <div style="font-weight:600">${gr.navn}</div>
          <div style="font-size:11px;color:var(--muted)">${gr.utlant_til}</div>
        </td>
        <td class="tc fw" style="white-space:nowrap">${gr.antall > 1 ? gr.antall + " stk" : "1 stk"}</td>
        <td class="tc mono small-text ${forfalt ? "orange-text fw" : ""}" style="white-space:nowrap">${gr.innleveringsdato}${forfalt ? " ⚠️" : ""}</td>
        <td class="tc" style="white-space:nowrap">
          <button class="btn-action btn-qr"  style="padding:8px 12px;font-size:15px;min-width:40px" onclick="leverGjenstander('${idStr}', ${gr.antall})">✅</button>
          <button class="btn-action btn-edit" style="padding:8px 12px;font-size:15px;min-width:40px" onclick="leverGjenstander('${idStr}', 1)">1</button>
        </td>
      </tr>`;
    }).join("");
  }
