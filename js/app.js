...
function lagQrCanvas(url, size) {
  return new Promise(resolve => {
    ...
    new QRCode(wrap, { text: url, width: size, height: size, colorDark: "#000000", colorLight: "#ffffff", correctLevel: QRCode.CorrectLevel.M });
    setTimeout(() => {
      const canvas = wrap.querySelector("canvas");
      if (canvas) {
        const dataUrl = canvas.toDataURL("image/png");
        document.body.removeChild(wrap);
        resolve(`<img src="${dataUrl}" width="${size}" height="${size}" style="display:block">`);
      } else {
        const img = wrap.querySelector("img");
        const src = img ? img.src : "";
        document.body.removeChild(wrap);
        resolve(`<img src="${src}" width="${size}" height="${size}" style="display:block">`);
      }
    }, 120);
  });
}
...
async function skrivUtEtikett() {
  ...
  const qrHTML = await lagQrCanvas(url, 175);
...
async function skrivUtAlle() {
  ...
  for (const id of printKo) {
    ...
    const url = APP_BASE_URL + "item.html?id=" + enc(id);
    const qrHTML = await lagQrCanvas(url, 175);
    parts.push(etikettHTML(g, qrHTML));
  }
  document.getElementById("printArea").innerHTML = parts.join("");
  await triggerPrint();
  btn.disabled = false; btn.textContent = "🖨️ Skriv ut alle";
}

// Show printArea (removes display:none), print, then hide again
