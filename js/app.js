var KATEGORIER = ["Camping","Kjokken","Sikkerhet","Utstyr","Navigasjon","Verkto","Belysning","Vann","Kommunikasjon","Annet"];
var STATUSER   = ["Tilgjengelig","Utlant","Til reparasjon","Tapt"];
...
// ---- HELPERS ----
function sk(s) {
  var m = {"Tilgjengelig":"tilgjengelig","Utlant":"utlant","Til reparasjon":"reparasjon","Tapt":"tapt"};
...
function oppdaterStats() {
  ...
  document.getElementById("stat-utlant").textContent       = gjenstander.filter(function(g) { return g.status === "Utlant"; }).length;
}
...
async function raskStatus(id, sel) {
  ...
  if (ny === "Utlant" && !g.utlant_til) { apneRediger(id, "Utlant"); sel.value = g.status; return; }
  var ok = Object.assign({}, g, {status: ny});
  if (ny !== "Utlant") { ok.utlant_til = ""; ok.utlansdato = ""; ok.innleveringsdato = ""; }
...
function toggleUtlan(s) {
  document.getElementById("utlanSection").style.display = s === "Utlant" ? "block" : "none";
}
...
function viseSammendrag() {
  ...
  var utl   = gjenstander.filter(function(g) { return g.status === "Utlant"; }).length;
  ...
  var forf  = gjenstander.filter(function(g) { return g.status === "Utlant" && g.innleveringsdato && new Date(g.innleveringsdato) < new Date(); }).length;
  var kpis = [
    {icon:"&#128230;", val:total, label:"Totalt",       color:"#46bdc6"},
    {icon:"&#9989;",   val:tilg,  label:"Tilgjengelig", color:"#34d399"},
    {icon:"&#128228;", val:utl,   label:"Utlant",       color:"#f87171"},
  ...
  KATEGORIER.forEach(function(kat) {
    ...
    var kU = items.filter(function(g) { return g.status === "Utlant"; }).length;
  ...
  var ul = gjenstander.filter(function(g) { return g.status === "Utlant"; });
...
async function lagreNyBoks() {
  ...
  if (!hylle) { visBanner("Hylleplassering er pakrevd", "error"); return; }
  if (!id)    { visBanner("Fyll inn hylleplassering for a generere ID", "error"); return; }
  if (!navn)  { visBanner("Navn er pakrevd", "error"); return; }
