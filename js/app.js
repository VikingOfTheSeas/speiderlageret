  ...
  return new Promise(resolve => {
    ...
    setTimeout(() => {
      ...
      if (canvas) {
        ...
        resolve(`<img src="${dataUrl}" width="280" height="280" style="display:block">`);
      } else {
        const img = wrap.querySelector("img");
        const src = img ? img.src : "";
        document.body.removeChild(wrap);
        resolve(`<img src="${src}" width="280" height="280" style="display:block">`);
      }
    }, 120);
  });
}
...
async function skrivUtEtikett() {
  ...
  const qrHTML = await lagQrCanvas(url, 280);
