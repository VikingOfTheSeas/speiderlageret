...
function visTabell(liste) {
  ...
  tbody.innerHTML = liste.map(g => {
    ...
    return `<tr class="item-row row-${sk(g.status)}${sel ? " row-selected" : ""}" data-id="${g.id}">
      ...
      <td class="tc col-extra small-text">${g.utlant_til || '<span class="dash">—</span>'}</td>
      <td class="tc col-extra muted-text">${g.kategori || "—"}</td>
      <td class="tc col-extra mono muted-text small-text">${g.serienummer || "—"}</td>
      <td class="tc col-extra mono small-text">${g.hylleplassering || '<span class="dash">—</span>'}</td>
    </tr>`;
  }).join("");
}
