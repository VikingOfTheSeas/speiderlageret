...
let maks = 0;
const re = new RegExp(”^” + pref.replace(/[.*+?^${}()|[]\]/g, “\$&”) + “-(\d+)$”, “i”);
gjenstander.forEach(g => { const m = g.id.match(re); if (m) maks = Math.max(maks, parseInt(m[1])); });
return maks + 1;
}
function autofyllHylle() {
const boksId = document.getElementById(“itemBoksId”) ? document.getElementById(“itemBoksId”).value : “”;
...
const start = parseInt(startEl ? startEl.value : 1) || 1;
const vis   = Array.from({ length: Math.min(3, ant) }, (*, i) => `${pref}-${String(start + i).padStart(3, "0")}`);
