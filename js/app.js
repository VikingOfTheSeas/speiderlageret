...
function apneNyBoks() {
  ...
  document.getElementById("nyBoksBildePrev").style.display = "none";
  document.getElementById("nyBoksBildeStatus").style.display = "none";
  document.getElementById("nyBoksBildeInput").value = "";
...
function genererBoksId() {
  ...
  if (!hylle) { document.getElementById("nyBoksId").value = ""; return; }
  // Format: B-A01 from A1, B-B03 from B3, B-A12 from A12
  const m = hylle.match(/^([A-Z]+)(\d+)$/);
  let base;
  if (m) {
    base = "B-" + m[1] + m[2].padStart(2, "0");
  } else {
    base = "B-" + hylle.replace(/[^A-Z0-9]/g, "").slice(0, 6);
  }
  // Ensure uniqueness — append counter if already exists
  let id = base;
  let n = 2;
  while (bokser.find(b => b.id === id)) { id = base + "-" + n; n++; }
  document.getElementById("nyBoksId").value = id;
}
...
async function lagreNyBoks() {
  ...
  if (!id)    { visBanner("Generer ID ved å fylle inn hylleplassering", "error"); return; }
  ...
  if (error) { visBanner("Feil: " + error.message, "error"); }
    ...
    lukkNyBoks();
    await lastInnBokser();
    visBanner("✓ Boks opprettet!", "success");
    settFane("bokser");
  }
...
function nesteNummer(pref) {
  // Find highest existing number for this prefix across all gjenstander
  let maks = 0;
...
function oppdaterPrev() {
  ...
  // Auto-update start number based on existing IDs with this prefix
  const startEl = document.getElementById("itemStartNr");
  if (startEl && pref.length >= 2) {
    const neste = nesteNummer(pref);
    startEl.value = neste;
  }
