...
async function startKamera() {
  ...
  try {
    if (!window.ZXing) throw new Error("ZXing ikke lastet");
    ...
    const backCam = devices.find(d =>
      /back|environment|rear|bak/i.test(d.label)
    ) || devices[devices.length - 1];

    const deviceId = backCam ? backCam.deviceId : undefined;

    // ZXing streams directly to the video element and fires callback on each decode
    await zxingReader.decodeFromVideoDevice(deviceId, video, (result, err) => {
      if (!result) return;
      behandleQrResultat(result.getText());
    });

    // Also ask for high-res back camera via constraints for sharpness
    if (video.srcObject) {
...
function behandleQrResultat(url) {
  ...
  const g = gjenstander.find(x => x.id === id);
  if (!g) { leggTilScanLog("Ukjent ID: " + id, true); return; }
