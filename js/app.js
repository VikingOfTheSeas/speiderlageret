...
async function visBokser() {
  ...
  tbody.innerHTML = bokser.map(b => {
    ...
    // Bygg nedbrutt liste — muted, liten
    const breakdown = [...navnMap.entries()]
      .map(([navn, r]) => {
        const farge = r.tilgj === 0 ? "color:var(--danger)" : r.tilgj < r.tot ? "color:var(--warning)" : "";
        return `<span style="white-space:nowrap;${farge}">${r.tilgj}/${r.tot} ${navn}</span>`;
      })
      .join('<span style="color:#2d3748"> · </span>');

    const gjenstanderCell = totalt === 0
      ? `<span class="muted-text">Tom</span>`
      : `<div style="display:flex;flex-direction:column;gap:3px;align-items:flex-end">
          <div><span class="green-text fw">${tilgj}</span><span class="muted-text">/${totalt} stk · ${typer} type${typer !== 1 ? "r" : ""}</span></div>
          <div style="font-size:10px;color:#475569;text-align:right;line-height:1.5">${breakdown}</div>
        </div>`;
    ...
    return `<tr class="item-row">
      ...
      <td style="text-align:right;padding-right:12px">${gjenstanderCell}</td>
      <td class="tc">
        <div class="actions-cell">
