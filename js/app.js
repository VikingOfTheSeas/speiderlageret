...
function apneRediger(id, forceStatus) {
  ...
  const erBulkEl = document.getElementById("editErBulk");
  if (erBulkEl) erBulkEl.checked = !!g.er_bulk;
  const antTotEl = document.getElementById("editAntallTotalt");
  if (antTotEl) antTotEl.value = g.antall_totalt || 1;
  const antUtlEl = document.getElementById("editAntallUtlant");
  if (antUtlEl) antUtlEl.value = g.antall_utlant || 0;
  const bulkFieldsEl = document.getElementById("bulkFields");
  if (bulkFieldsEl) bulkFieldsEl.style.display = g.er_bulk ? "block" : "none";
  populerBoksSelect();
...
async function lagreRediger() {
  ...
  const erBulk = document.getElementById("editErBulk")?.checked || false;
  const oppdatert = {
    id: editingId,
  ...
  if (error) { visBanner("Feil: " + error.message, "error"); }
    ...
    if (status === "Utlånt" && utlant_til && !varUtlant) {
      await db.from("utlanslogg").insert({
        ...
        antall: 1, utlant_til, utlansdato,
        innleveringsdato: innlevering || null, status: "aktiv"
      });
    }
    if (varUtlant && status !== "Utlånt") {
      await db.from("utlanslogg")
        .update({ status: "levert", levert_dato: new Date().toISOString().split("T")[0] })
        .eq("gjenstand_id", editingId).eq("status", "aktiv");
    }
    await lastInn(); lukkRedigerModal(); visBanner("✓ Lagret!", "success");
  }
  btn.disabled = false; btn.textContent = "💾 Lagre";
}
...
async function leggTil(e) {
  ...
  if (error) { visBanner("Feil: " + error.message, "error"); }
    ...
    const addBulkFields = document.getElementById("addBulkFields");
    if (addBulkFields) addBulkFields.style.display = "none";
    const itemHylle = document.getElementById("itemHylle");
    if (itemHylle) itemHylle.value = "";
    oppdaterPrev();
    await lastInn();
    lukkLeggTilModal();
  }
...
function toggleAddBulk(cb) {
  const el = document.getElementById("addBulkFields");
