...
function renderBoks() {
  ...
  if (!grupper.size) {
    ...
    tbody.innerHTML = [...grupper.entries()].map(([navn, gr]) => {
      ...
      return `<tr>
        <td><a class="item-name-link" href="item.html?id=${enc(gr.alle[0].id)}">${navn}</a></td>
        ...
        <td class="tc">
          <div class="actions-cell">
            <button class="btn-action btn-edit" onclick="apneUtlanForNavn('${sn}')">📤</button>
            <button class="btn-action btn-qr"  onclick="apneLeverForNavn('${sn}')">📥</button>
          </div>
        </td>
      </tr>`;
    }).join("");
  ...
  uTbody.innerHTML = aktiveUtlan.length === 0
    ...
    : aktiveUtlan.map(u => {
        ...
        return `<tr class="${forfalt ? "row-forfalt" : ""}">
          ...
          <td class="tc">
            <button class="btn-action btn-qr"  onclick="leverEnkelt(${u.id}, ${u.antall})">✅ Alle</button>
            <button class="btn-action btn-edit" onclick="leverDels(${u.id})">✏️</button>
          </td>
        </tr>`;
      }).join("");
}
