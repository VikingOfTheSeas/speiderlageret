...
async function skrivUtAlle() {
  ...
  document.getElementById("printArea").innerHTML = sider.map((side, idx) => {
    const erSiste = idx === sider.length - 1;
    const stil = erSiste ? "" : ' style="height:278mm"'; // 8×33mm + 7×2mm gaps
    return `<div class="print-side${erSiste ? "" : " print-side-break"}"${stil}>${side.join("")}</div>`;
  }).join("");
